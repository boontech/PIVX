...
boondevcoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<boondevcoin_address\> \<monthly_payment_boondevcoin\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ boondevcoin, _12_ times over the course of a year totaling _24000_ boondevcoin.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<boondevcoin_address\> \<monthly_payment_boondevcoin\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` boondevcoin.
 ...
 * vote-many          - Vote on a boondevcoin initiative
 * vote-alias         - Vote on a boondevcoin initiative
 * vote               - Vote on a boondevcoin initiative/budget
